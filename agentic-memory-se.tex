% AI4SE Workshop @ KI 2026 — Camera-Ready Version
% Springer LNCS format — 8 pages text + references
...
understanding. In particular, they lack episodic memory, i.e.\ structured records of past sessions, debugging trajectories, and decision rationale, as well as temporal versioning, i.e.\ the ability to track when a piece of knowledge was valid, when it changed, and when it became stale. We present a survey that synthesizes
...
ecosystem of add-ons based on the Model Context Protocol (MCP) has emerged to fill the
...
session. Yet that knowledge vanishes when the session ends, i.e. the next
invocation begins from scratch, repeating investigations, rediscovering
constraints, and re-learning project conventions. Recent production systems have begun to address this gap, e.g.\ Claude Code persists automatically extracted memories across sessions and Copilot injects repository-level custom instructions as reusable project context~\cite{agentreadmes2025}, but these mechanisms remain limited in scope and depth.

The field has responded rapidly~\cite{hu2025survey,memagents2026}, with every major AI provider now shipping
some form of persistent memory. Yet no production harness implements episodic memory or temporal versioning natively. A thriving ecosystem of community-built MCP add-ons~\cite{mcpmemory2026} has emerged, evidence that practitioners feel the deficit acutely, but fundamental properties such as team-shared memory or decision provenance are only rarely addressed.

In this paper, we survey and classify memory architectures for software engineering agents. Specifically, we i)~conduct a feature-level analysis of ten production software engineering harnesses and ten representative MCP-based community add-ons across 20~dimensions; ii)~classify memory architectures from the recent research literature; and iii)~identify open gaps at the intersection of memory research and software engineering practice. Our goal is to provide a structured overview of the current state of the art and to highlight where the most impactful research and engineering efforts are needed.
...
We selected ten production software engineering agent harnesses (see Table~\ref{tab:ecosystem}) based on three criteria: i)~general availability as of June~2026, ii)~active development with at least one major update in the preceding six months, and iii)~widespread adoption. From over 40~MCP-based memory servers, we selected ten representative examples spanning different CoALA memory types. Each system was evaluated through official documentation and publications, hands-on testing where accessible, and community reports. Capabilities were rated as full support, partial support, or absent; ambiguous cases were rated conservatively.

We evaluate along 20~dimensions grouped into six categories. The first three, \emph{CoALA memory types}, \emph{memory operations}, and \emph{temporal mechanisms}, are derived from the CoALA~\cite{sumers2024} and Hu~et~al.~\cite{hu2025survey} taxonomies. The remaining three, \emph{SE-specific capabilities}, \emph{collaboration \& governance}, and \emph{integration}, extend the evaluation to practical concerns specific to software engineering workflows. Table~\ref{tab:ecosystem} summarizes the comparison. All ten harnesses support human-written instruction files, representing declarative semantic memory, with widespread adoption~\cite{agentreadmes2025}, yet little resolution improvement~\cite{gloaguen2026}. Context
...
No production system supports temporal versioning or episodic memory. Notably, those memory capabilities most critical for software engineering agents that
learn over time are those that no tool implements. This gap has driven a community response, as described next.
...
\subsection{Community Extensions via MCP}\label{sec:mcp}

The gap between what production harnesses ship and what practitioners need has
spawned a rapidly growing ecosystem of community-built memory
extensions via MCP~\cite{mcpmemory2026}. Our survey identifies over 40 MCP-based
...
attention-based, and embedding-based methods~\cite{hu2025survey}. Cross-cutting learning
...
cognitive memory evaluation~\cite{gurawaliya2026}, i.e. extraction-based, self-managing, and graph-based temporal. Table~\ref{tab:architectures} summarizes their key properties and relates them to the evaluation dimensions of Table~\ref{tab:ecosystem}.
...
\emph{factual recall} and correspond to the \emph{semantic memory} and \emph{factual knowledge} columns in Table~\ref{tab:ecosystem}. For software engineering tasks requiring explicit knowledge retrieval, this paradigm
...
state, corresponding to the \emph{experiential knowledge} and \emph{auto-generated memories} dimensions in Table~\ref{tab:ecosystem}. For software engineering agents that must maintain an evolving picture of project
...
\emph{bi-temporal} approach is unique among production memory systems and directly addresses the \emph{temporal versioning} and \emph{bi-temporal validity} dimensions that remain near-universally absent in Table~\ref{tab:ecosystem}.
...
semantic, temporal, causal, and entity graphs with policy-guided traversal, offering fine-grained control relevant for software engineering, e.g.\ querying dependency relationships, decision provenance, or API validity independently. A formal theory of hierarchical memory~\cite{hierarchicalmemory2026} and its implementation TiMem~\cite{timem2026} define extraction, coarsening, and traversal operators over temporal memory trees, potentially essential for agents processing long execution traces. Hybrid local/cloud architectures are also emerging: LightMem~\cite{lightmem2026} delegates routing and filtering to small local models while HyMem~\cite{hymem2026} achieves comparable performance at reduced cost via dynamic two-tier retrieval.
...
\begin{table}[t]
\centering
\includegraphics[width=\textwidth]{figures/architecture-table.png}
\caption{Comparison of memory architecture paradigms across key properties relevant to software engineering agents.}
...
Beyond the general-purpose memory architectures compared in Table~\ref{tab:architectures}, several mechanisms have
been developed specifically for or with direct relevance to software engineering agents. These address capabilities that remain largely absent in the production landscape surveyed in Table~\ref{tab:ecosystem}, particularly temporal versioning, decision provenance, and cross-session learning.
...
The SWE Context Bench~\cite{zhu2026} evaluates whether agents benefit from prior experience with one critical finding, i.e. summarized experience improves resolution, while unfiltered experience hurts. Several approaches address this through structured learning from trajectories. Experiential Co-Learning~\cite{qian2024} mines historical trajectories, Experiential Reflective Learning~\cite{erl2026} distills transferable heuristics, and Subtask-Level Memory~\cite{subtaskmem2026} aligns storage and retrieval at the subtask rather than instance level. MemGrad~\cite{memgrad2026} combines retrospective failure-mode capture with prospective gradient-derived strategies.
Procedural memory, such as Mem$^{\mathrm{p}}$~\cite{memp2025}, distills trajectories into step-by-step instructions and higher-level abstractions, with procedural knowledge transferring effectively from stronger to weaker models. ReasoningBank~\cite{reasoningbank2025} distills generalizable reasoning strategies from self-judged successes and failures, while persistent note-taking~\cite{cca2025} records cross-session hindsight notes. Live-SWE-agent~\cite{xia2025} self-creates new tools during execution, representing procedural memory through self-modification.
Notably, WebCoach~\cite{webcoach2026} reveals a capacity threshold, i.e. 7B models do not benefit from experiential memory, while 32B+ models show pronounced gains, and self-generated experiences outperform externally seeded ones~\cite{erl2026}.
...
exhibits a strong single-agent bias~\cite{memagents2026}. Collaborative Memory~\cite{rezazadeh2025} provides multi-user memory sharing with private and shared tiers, where each fragment carries immutable provenance attributes. This maps naturally onto software engineering teams, i.e.\ private developer context versus shared project knowledge. However, MINJA~\cite{minja2026} demonstrates that shared-memory agents are vulnerable to injection attacks, a critical supply-chain risk for agents with codebase access. SSGM~\cite{ssgm2026} and MemArchitect~\cite{memarchitect2026} propose governance middleware, while the mnemonic sovereignty survey~\cite{mnemonic2026}
addresses EU AI Act compliance. Yet no approach targets software engineering-specific governance
needs such as repository-aligned access control or temporal policies tied
to release cycles.


%%% ==================================================================
\section{Discussion}\label{sec:gaps}
%%% ==================================================================
...
a manual integration challenge.

Our analysis has limitations as it represents a snapshot as of mid~2026 in a rapidly evolving landscape, evaluations in Table~\ref{tab:ecosystem} are based on documentation and hands-on testing rather than controlled experiments, and our selection of MCP add-ons targets representativeness rather than completeness. Nevertheless, the convergent evidence across production harnesses, community add-ons, and research literature consistently points to the same structural deficits. Closing these gaps will require a
